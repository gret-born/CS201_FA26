...
penguins_without_comments <- penguins %>%
  select(-comments)
...
number8 <- penguins %>%
  ...
  summarize(culment_depth_means = mean(culmen_depth_mm, na.rm = TRUE), culment_length_mean = mean(culmen_length_mm, na.rm = TRUE))
...
case_when <- penguins %>%
  mutate(size = case_when(body_mass_g >= 4050 ~ "big", body_mass_g < 4050 ~ "small"))
...
final_boss <- penguins %>%
  filter(sex == "female", !is.na(flipper_length_mm), island %in% c("Dream", "Biscoe")) %>%
 ####idk :(
```
